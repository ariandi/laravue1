		...
		<table class="table table-bordered">
			...
			<tbody>
				<tr v-for="(user, index) in users">
					...
					<td>{{ user.name }}</td>
					<td>{{ user.email }}</td>
					<td><a :href="'/users/'+user.id+'/edit'" class="btn btn-primary btn-sm">Edit</a> || <a href="javascript:;" class="btn btn-danger btn-sm" v-on:click="deleteUser(user.id, index)">Delete</a></td>
				</tr>
			</tbody>
		</table>
